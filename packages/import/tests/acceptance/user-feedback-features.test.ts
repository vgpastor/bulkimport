...
import type { DomainEvent, ParsedRecord } from '@batchactions/core';

// ============================================================
// #1 — job:started fires asynchronously
// ============================================================
describe('job:started deferred emission', () => {
  it('should allow handlers registered after start() on the same tick to receive job:started', async () => {
    ...
    importer.on('job:started', handler);

    await promise;

    expect(handler).toHaveBeenCalledOnce();
    expect(handler).toHaveBeenCalledWith(expect.objectContaining({ type: 'job:started' }));
  });

  it('should still deliver job:started to handlers registered before start()', async () => {
    const csv = 'email,name\nuser@test.com,Alice';
...
describe('onAny wildcard event subscription', () => {
  ...
  it('should receive both onAny and specific on() events without duplication', async () => {
    ...
    importer.on('job:started', specificHandler);
    ...
    // Wildcard receives ALL events, including job:started
    expect(wildcardHandler.mock.calls.length).toBeGreaterThan(1);
    expect(wildcardHandler).toHaveBeenCalledWith(expect.objectContaining({ type: 'job:started' }));
  });
});
