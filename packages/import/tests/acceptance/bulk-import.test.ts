...
describe('Test 1: Full happy path', () => {
  it('should process all records in batches and emit correct events', async () => {
    ...
    importer.on('job:started', () => events.push('job:started'));
...
describe('Test 10: Granular events in correct order', () => {
  it('should emit events in the correct lifecycle order', async () => {
    ...
    importer.on('job:started', () => eventOrder.push('job:started'));
    ...
    importer.on('job:completed', () => eventOrder.push('job:completed'));
    importer.on('job:progress', () => eventOrder.push('job:progress'));
    ...
    // Verify order
    expect(eventOrder[0]).toBe('job:started');
    expect(eventOrder[eventOrder.length - 1]).toBe('job:completed');
    ...
    expect(eventOrder.filter((e) => e === 'job:progress')).toHaveLength(2);
  });
});
